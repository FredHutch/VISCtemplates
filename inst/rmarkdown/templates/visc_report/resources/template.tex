...
\usepackage{caption} % for improving look of table captions when longtable is used
...
% note that the value of use_old_csl_refs is set aumotically in
% visc_pdf_document()
$if(use_old_csl_refs)$
% older version of CSLReferences
...
$else$
% newer version of CSLReferences that uses a list environment with bibitems
...
%\captionsetup{width=16cm} % fix caption widths for longtable (narrow by default)
