...
of the baseline background-adjusted MFI minus background-adjusted blank (MFI*) 
by antigen. 
...
* MFI* values at the screening dilution were greater than the maximum of 100 or 
the 95th percentile of baseline samples (by antigen).
* MFI* values were greater than 3 times the baseline background-adjusted MFI 
minus baseline background-adjusted blank values.
* Background-adjusted MFI values were greater than 3 times the baseline 
...
Log$_{10}$ background- and blank-subtracted MFI (MFI*) was used to compare 
...
To measure durability, the fold-change in response magnitude [MFI* or AUTC] 
...
MFI* values were truncated at 1 for AUC calculations.
...
The antigen-specific MFI* value of each non-responder was set to 100.
...
Graphical distributions of response magnitudes [MFI\*, AUTC, AUC-MB, etc] were 
...
The magnitudes were plotted on a log$_{10}$ scale. AUTC and MFI* lower than 100 
...
MB curves were stratified by ARP panels (i.e., gp140, gp120 and V1V2) 
...
median MFI* or AUTC over time by antigen. 
MFI* and AUTC values less than 100 were truncated at 100 for plotting purposes. 
