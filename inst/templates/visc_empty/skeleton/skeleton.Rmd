...
# ensure packages referenced with :: in this file are loaded with library calls
# before calling get_session_info()
pkg_namespaces_used <- detect_namespaces_in_text(readLines(knitr::get_current_input()))
for (pkg in pkg_namespaces_used) {
  if (!any(installed.packages()[,1] == pkg)) {
    stop(paste(pkg, "package is used but has not been loaded with a library() call"))
  }
}
