...
<!-- The STP should create a table of missing concentrations in the QC report. You may want to further investigate patterns of missingness here. In many of our human studies, NA implies BLOQ, so this needs to be checked for each protocol. -->
...
pk_data %>% 
  filter(visit_type == "Check the study schema to see which visit_type corresponds to baseline.") %>% 
...
cvdxxx_elisa_preprocess <- pk_data %>%
  ...
  mutate(
    dose_amount = case_when(
      dose_units == "mg/kg" ~ dose*weight, # weight-dependent dose
      dose_units == "mg" ~ dose # fixed dose
    ),
...
monolix_data <- cvdxxx_elisa_preprocess %>% 
  ...
  group_by(analyte, pubid, infusion_no) %>% 
  mutate(
