...
#
# 1) `install_git()` doesn't work well with network drives, so we use
# `VISCtemplates::install_git_workaround()`. Note that in Rmd files, `setwd()`
# only impacts the current code chunk.
#
...
install_git_workaround(
  networks_path('cavd', 'Studies', 'cvdNNN', 'pdata', 'cvdNNN.git'),
  ref = 'main',
  git = 'external'
)
...
# Standard data object naming, e.g. package named `cvdNNN` and object name
# starts with `cvdNNN_`. The package name is parsed automatically from object
# name
exampleData_ICS <- visc_load_pdata(
  'cvdNNN_ics',
  proj_or_datapackage = 'datapackage',
  criteria = 'a70bc33ea70bc33ea70bc33ea70bc33e'
)

# Non-standard data object naming. e.g., package named `cvdNNN` object name
# starts with `other`. Package and object names are parsed independently.
exampleData_ICS <- visc_load_pdata(
  'other_ics',
  proj_or_datapackage = 'datapackage',
  criteria = 'a70bc33ea70bc33ea70bc33ea70bc33e',
...
```{r example-plot-loop, results="asis", eval = T}

for(i in c("GAG", "POL")){
  
  # NOTE the use of the limits argument in scales_...() to maintain consistent legends
  
  sub_plt_data <- filter(plt_data, Stim == i)
  
  sub_plt <- sub_plt_data |>
    ggplot(aes(x = Group,
               y = pmax(PercentCellNet, 0.005),
               shape = response_and_group,
               color = response_and_group)) +
    geom_point(position = position_jitter(width = .25, height = 0, seed = 3241),
               size = 1) +
    scale_color_visc(limits = unique(plt_data$response_and_group)) +
    scale_shape_visc(limits = unique(plt_data$response_and_group)) +
    geom_boxplot(
      data = sub_plt_data |> filter(response == 1),
      fill = NA, lwd = .5, outlier.colour = NA
      ) +
    scale_y_log10(
      breaks = c(.005,.01,.05,.25,1,2, 4, 10),
  ...
  insert_fig_subchunk(
    fig = sub_plt,
    fig_chunk_name = paste0("fig-", tolower(i)),
    fig_caption_short = paste0("Looped short caption for ", i),
    fig_caption_long = paste0("Looped long caption for ", i, ". With it's special character examples: $\\alpha < 5 %$ or $\\alpha \\le 5$ %. ")
  )
