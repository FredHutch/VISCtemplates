...
```{r example-tab, results="asis", warning=kable_warnings, eval=(output_type == 'latex')}
...
```{r, results="asis", eval=(output_type != 'latex')}

# NOTE: this is the same table as above, but using flextable instead of kable for Word output only
magnitude_tab %>%
...
EDIT ME: there are duplicated tables below, to illustrate the use of both kable() and flextable().
Note that the kable() tables will be formatted poorly in Word document outputs, due to limitations of the kableExtra package.
...
# this creates the software session information table using kable() and kableExtra
# NOTE: formatting will look bad in Word output, but good in PDF output
kable(
...
# this creates the software session information table using flextable
# NOTE: formatting should look good in both Word and PDF outputs
...
# this creates the software package version information table using kable() and kableExtra
# NOTE: formatting will look bad in Word output, but good in PDF output
kable(
...
# this creates the software package version information table using flextable
# NOTE: formatting should look good in both Word and PDF outputs
