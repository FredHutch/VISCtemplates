...
All statistical analysis was performed using R statistical software [@rlang],
including packages from the tidyverse [@tidyverse].
For details, see Tables `r insert_ref("tab:Software-Session-Information")` and
`r insert_ref("tab:Software-Package-Version-Information")`.

<!-- Note: DO NOT EDIT the following two paragraphs -->
<!-- You may, however, add new paragraphs with further information if needed -->

VISC PT reports are generated and reviewed by humans with the support of AI tools. AI assistance is used to draft or refine code that is run by VISC members to process and analyze study data, with VISC members carefully reviewing, validating, and corroborating any output or results generated by the application of such AI-augmented code to study data. Confidential and restricted-use study data are not used as input to any AI systems. AI systems are also not used for drafting text related to study results, as this would require exposing sensitive information to potentially non-secure AI tools.

Pursuant to current policies governing workforce members at Fred Hutch Cancer Center, VISC uses Microsoft Copilot Chat, powered by GPT-4o, to assist with report coding. Microsoft Copilot Chat is an AI tool with enterprise-grade security, privacy and built-in compliance. Fred Hutch enterprise data protection applies to prompts and responses, which are stored securely so that information from chat conversations does not get exposed to the public.
